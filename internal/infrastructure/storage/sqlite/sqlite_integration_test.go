...
func TestIntegration_SessionStore_ExpiredToken(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "returns ErrUnauthorized and deletes the expired session on validate",
			run: func(t *testing.T, db *sql.DB) {
				ctx := context.Background()
				token := "expired-token-abc"
				if _, err := db.ExecContext(ctx,
					`INSERT INTO sessions (token, operator_address, expires_at) VALUES (?,?,?)`,
					token, addr1, "2000-01-01T00:00:00Z",
				); err != nil {
					t.Fatalf("insert expired session: %v", err)
				}

				store := NewSessionStore(db)
				if _, err := store.Validate(ctx, token); !errors.Is(err, ports.ErrUnauthorized) {
					t.Errorf("expected ErrUnauthorized for expired token, got %v", err)
				}

				var count int
				db.QueryRowContext(ctx, `SELECT COUNT(*) FROM sessions WHERE token=?`, token).Scan(&count)
				if count != 0 {
					t.Error("expected expired session to be deleted on validate")
				}
			},
		},
	}
